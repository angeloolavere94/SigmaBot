...
def create_profile_card(
    ...
    try:
        ...
        font_display = ImageFont.truetype("arial.ttf", 40)
        font_body = ImageFont.truetype("arial.ttf", 35)
        font_label = ImageFont.truetype("arialbd.ttf", 35)
    except:
        try:
    ...
    draw.text((text_x, pfp_y + 70), display_name, font=font_display, fill=text_color)
...
async def afk(interaction: discord.Interaction, reason: str = None):
    ...
    embed = discord.Embed(
        title="AFK",
        description=f"{interaction.user.mention} has been set to AFK!",
        color=0xFFA500
    )
